        ...
        WiringUI.history(cart.currentPage) {
          (old, nw, ns) => {
            ...
            def html(ci: BookItem): NodeSeq = {
              ...
              ( "tr [id]" #> ciToId(ci) &
               "@title *" #> <i>{ci.item.title}</i> &
               "@img *" #> <a class="thumbnail" href={ci.item.thumbUrl.get}> <img src = {ci.item.imgUrl.get} width={"90"} height={"120"} alt="Image Loading" title="Click to view book at the Internet Archive"/>
               <span><img src={ci.item.imgUrl.get} width={"300"} height={"500"}></img></span> </a> &
