          ...
          WiringUI.history(TheCart.currentDocPage) {
            (old, nw, ns) => {
              ...
              def html(i: PageItem): NodeSeq = {
                ( "tr [id]" #> ciToId(i) &
                 "@name *" #> <u><strong>{scala.xml.Unparsed(i.item.title.get)}</strong></u>
                 <p>{scala.xml.Unparsed(Librarian.library.tagTerms(i.item.summary.get))}</p>
                 ...
                 <span> | </span>
                 {LogServer.logLink(i.item.externalUrl.get, "[View page at Internet Archive]",
                                          S.session.openOr("NONE").toString + " --> Archive Link Clicked (" + i.item.externalUrl.get + ")")}</span> &
                 "@img *"  #> <a class="thumbnail" href="#thumb"><img src={i.item.thumbUrl.get} width={"90"} height={"120"} />
                    <span><img src={i.item.imgUrl.get} width={"450"} height={"700"}></img></span></a>
                )(theTR)
