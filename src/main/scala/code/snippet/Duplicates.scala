...
//                catch { case ex: Exception =>  print("Not found in dupe index"); ex.printStackTrace; Nil}
    ...
    val rlist = Librarian.library.getOverlaps(thebook.item.id, ProteusType.Collection).get().results.toList
...
//      var decoded = urlDecode(dupeQuery.toString)
    ...
    // Librarian.library.getOverlaps(thebook.item.id, ProteusType.Collection).get.getResultsList.asScala
      val foundItem = TheCart.get.findBookItem(id.toInt).item
      val dList = Librarian.library.getOverlaps(foundItem.id, ProteusType.Collection).get().results.toList
      // Add these results back into the cart/book bag
      dList.foreach(d => TheCart.addItem(d))
      val rlist = dList.map(d => TheCart.get.findBookItem((d.id.identifier + d.id.resourceId).hashCode))
...
//          val order = getOrder(id, d.identifier)
          ...
          ".archLink" #> d.item.externalUrl.get &
          //[book cover] [title] [score] [bar image]
          ".topbar" #> <a class="thumbnail" href="#thumb"> <img src={d.item.thumbUrl.get} width={"60"} height={"100"} />
            <span><img src={d.item.imgUrl.get} width={"300"} height={"500"}></img></span> </a> &
          ".title1" #> {<span>(reference)</span>} &
          ".title2" #> {<a href={d.item.externalUrl.get}>{d.item.title}</a> } &
...
//    }
  ...
  def duplicateTitle = {
    ...
    ".showTitle" #> <span>Partial duplicates of <I>{baseDupe.item.title}</I></span> &
    ".minh" #> <a class="thumbnail" href="#thumb"> <img src ={baseDupe.item.thumbUrl.get} width={"85"} height={"150"}/> </a>
  }
