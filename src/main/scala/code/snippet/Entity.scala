...
//    annotations.foreach(a => log.info((userName :: a ::: page_metadata).map(v => "<" + v + ">").mkString(", ")))
  ...
  def listEntityResults = {
    ...
    val rlist = if (page.startsWith("page_")) {
        ...
        //val people = Librarian.library.getContents(pageItem.id, ProteusType.Page, ProteusType.Person).get.getResultsList.asScala.toList
        val people = Librarian.library.getContents(pageItem.id, ProteusType.Page, ProteusType.Person).get().results.toList
        val locations = Librarian.library.getContents(pageItem.id, ProteusType.Page, ProteusType.Location).get().results.toList
	      people ::: locations
	    } else {
    ...
    "#entities" #> rlist.map(ent => TheCart.get.findEntityItem((ent.id.identifier + ent.id.resourceId).hashCode)).map(ent =>
      "a *" #> <strong>{ent.item.title}</strong> &
      "a [href]" #> Entity.getEntityLink(ent.hashCode.toString, ent.hashCode.toString) &
      "@cat *"  #> {ent.item.proteusType.get.name.capitalize} &
  ...
  def getAdditionalInfo(result: EntityItem) : String = {
    if (result.item.proteusType.get.equals(ProteusType.Person)) {
      ...
      return result.item.proteusType.get.name.capitalize
    } else if (result.item.proteusType.get.equals(ProteusType.Location)) {
        val location = Librarian.library.lookupLocation(result.item).get()
        return "(" + location.longitude+ ", " + location.latitude + ")"
    } else return result.item.proteusType.get.name.capitalize
    ...
    if (isAmbiguous(id)) {
      ...
      try {
        val details = if(document.isPerson) {
	          ...
	          (person.languageModel.get.terms, person.wikiLink)
	        } else {
	          val loc = Librarian.library.lookupLocation(document.item).get()
	          (loc.languageModel.get.terms, loc.wikiLink)
	        } //document.terms.toList
        val terms = details._1
        val wiki = details._2.get
        
        ".enttype" #> "Ambiguous Entity" &
        ".entterm" #>  document.item.title &
        ...
        ".histogram *" #> terms.map { (B) => <tr><td>{B.term}</td> <td>{B.weight}</td></tr>}
      } catch {
...
//            ".entwiki *" #> <a href={"http://en.wikipedia.org/w/index.php?search="+ getTitle(id)}>Search Wikipedia</a>
    ...
    // to double quote string vars, ("\""*1)+getTitle(id)+("\""*1)
      ...
      val details = if(document.isPerson) {
	          ...
	          println("This shouldn't happen: ERROR: " + document.item.proteusType)
	          val org = Librarian.library.lookupOrganization(document.item).get()
	          (org.languageModel.get.terms, org.wikiLink)
	        }
      val terms = details._1
      val wiki = details._2.get
...
//      println("ENTITY: " + getTitle(id))
      ...
      ".desc" #> document.item.summary.get.text &
      ...
      ".entterm" #> document.item.title &
      ".entpic" #> <img src={document.item.imgUrl.get} /> &
